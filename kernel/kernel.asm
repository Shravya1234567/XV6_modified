...
void init_page_ref()
{
  ...
  initlock(&page_ref.lock, "page_reference");
...
int get_page_ref(void *pa)
{
  ...
  int ans;
  ans = page_ref.count[(uint64)pa >> 12];
...
int page_fault_handler(void *va, pagetable_t pagetable)
{
  ...
  if ((uint64)va >= MAXVA || (((uint64)va <= PGROUNDDOWN(process->trapframe->sp)) && (((uint64)va + PGSIZE) >= PGROUNDDOWN(process->trapframe->sp))))
  ...
  uint64 test;
  pte_t *ran;
  uint flag_var, flag_test;

  va = (void *)PGROUNDDOWN((uint64)va);
  ran = walk(pagetable, (uint64)va, 0);
...
00000000800033f8 <usertrap>:
{
  ...
  asm volatile("csrr %0, stval" : "=r" (x) );
    ...
    ans = page_fault_handler((void *)r_stval(), p->pagetable);
...
void syscall(void)
{
  ...
  if (brr < NELEM(syscalls) && brr > 0 && syscalls[brr])
  ...
  {
    prog->trapframe->a0 = syscalls[brr]();
    ...
    {
      printf("%d: syscall %s (", prog->pid, syscall_names[brr]);
